...
sub send_sms ($self, $to, $message, %opts) {
  ...
  my $ua = $self->ua;
  my $api_type = $config->{api_type} // 'cgi'; # Default to CGI, can be 'api' for JSON API
  ...
  if ($api_type eq 'api') {
    ...
    if ($to =~ /^00(\d+)/) {
      ...
      # If no +, add it
      $formatted_number = "+$to" if $to =~ /^\d+$/;
    }

    my $payload = {
      data => {
        modem   => $config->{modem} // '3-1',
    ...
    if ($tx->result->is_success) {
      ...
      if ($json && $json->{success}) {
        ...
        $response->{tx_id} = $json->{data}->{id} if $json->{data} && $json->{data}->{id};
        ...
        $response->{message} = 'Send failed: ' . ($json->{error} // 'Unknown error');
      }
    } else {
    ...
    if ($to =~ /^\+(\d+)/) {
      ...
      # If no +, assume it needs 00 prefix
    ...
    warn "SMS Send CGI URL: $full_url";
...
sub receive_sms ($self, %opts) {
  # This method is now just a wrapper around get_device_messages
  # for backward compatibility
  return $self->get_device_messages();
}
...
sub get_device_messages ($self) {
  my $config = $self->config;
  my $ua = $self->ua;
  my $api_type = $config->{api_type} // 'cgi';
  ...
  if ($api_type eq 'api') {
    ...
    if ($tx->result->is_success) {
      my $json = $tx->result->json;
      warn "SMS Device Messages API Response: " . encode_json($json);

      # Parse JSON response
      # Expected format: {"success": true, "data": [{"id": "123", "sender": "+46...", "message": "text", "timestamp": "..."}, ...]}
      if ($json && $json->{success} && $json->{data}) {
        for my $msg_data (@{$json->{data}}) {
          next unless $msg_data->{sender} && $msg_data->{message};

          my $sms = {
            phone     => $msg_data->{sender},
            message   => $msg_data->{message},
            timestamp => $msg_data->{timestamp} // $msg_data->{date},
            msg_id    => $msg_data->{id} // $msg_data->{index},
          };
      ...
      warn "SMS Device Messages API Request Failed: " . $tx->result->code . " - " . $tx->result->message;
    }
  } else {
    # Use traditional CGI endpoint
    ...
    warn "SMS Device Messages CGI URL: $full_url";
    my $tx = $ua->get($full_url);
    ...
    if ($tx->result->is_success) {
      ...
      warn "SMS Device Messages CGI Response: $body"; # Debug log
      ...
      warn "SMS Device Messages CGI Request Failed: " . $tx->result->code . " - " . $tx->result->message;
    }
  }
